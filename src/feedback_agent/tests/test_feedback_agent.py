...
from agent import run_feedback_agent, run_from_saved_feedback

REPO      = Path(__file__).parent.parent.parent.parent
USECASES  = REPO / "evals" / "usecases"
SAVED_FB  = REPO / "src" / "ahaa_app" / "saved_feedback"
CATALOGS  = REPO / "catalog_templates"
...
# ---------------------------------------------------------------------------
# Test 4 — run_from_saved_feedback bridge (app integration path).
#
# Uses the dummy saved_feedback folder (family_3_bundle_3_*).
# The folder was generated by the app: family_3, bundle_3, a reviewer who
# flagged trig_pay_stubs_checklist and wrote "we only require 3 consecutive
# pay stubs."  We pass in the same buggy TRIGGERS_T1 / REQS_T1 so compliance
# starts failing, then confirm the bridge produces a passing catalog.
# ---------------------------------------------------------------------------

def test_run_from_saved_feedback() -> None:
    saved_dirs = sorted(SAVED_FB.glob("family_3_bundle_3_*"))
    assert saved_dirs, f"No saved feedback folder found under {SAVED_FB}"
    folder = saved_dirs[0]

    # Verify pre-fix: buggy catalogs from T1 fail against the saved bundle
    family = json.loads((folder / "family_application.json").read_text())
    bundle = json.loads((folder / "document_bundle.json").read_text())
    pre_pass, _ = run_compliance(family, bundle, TRIGGERS_T1, REQS_T1)
    assert not pre_pass, "Expected pre-fix failure"
...
# ---------------------------------------------------------------------------
# Test 5 — req references wrong document year (2023 instead of 2024).
#
# family_1 (James Hartley): tax returns filed → trigger fires.
# bundle has federal_tax_return_2024.pdf but req looks for 2023 → FAIL.
# Human feedback: the year should be 2024.
# Expected fix: document_type changes to federal_tax_return_2024.pdf.
# ---------------------------------------------------------------------------
...
REQS_T5 = [
    {
        ...
        "description": "2024 federal tax return — bug: looks for 2023",
        "document_spec": {
            "type": "document",
            "document_type": "federal_tax_return_2023.pdf",
            "notes": None,
        },
        "source_reference": {
            "document": "Maple Square FCFS Application 2026",
...
FEEDBACK_T5 = (
    "The federal tax return requirement failed because the system required "
    "federal_tax_return_2023.pdf but the applicant submitted federal_tax_return_2024.pdf. "
    "This program cycle requires 2024 tax returns, not 2023. "
    "Please update req_federal_tax_return to require federal_tax_return_2024.pdf."
)


def test_wrong_tax_year_fix() -> None:
    family = _family(1)
    bundle = _bundle(1)

    trace = run_compliance_trace(family, bundle, TRIGGERS_T5, REQS_T5)
    assert not trace["passed"], "Expected pre-fix failure"
    assert any(
        r["requirement_id"] == "req_federal_tax_return" and not r["satisfied"]
        for r in trace["requirement_trace"]
    )

    updated_triggers, updated_reqs, analysis = run_feedback_agent(
        trigger_catalog=TRIGGERS_T5,
        req_catalog=REQS_T5,
        bundle=bundle,
        family=family,
        compliance_trace=trace,
        human_feedback=FEEDBACK_T5,
    )
    print(f"\n[T5] Analysis: {analysis}")

    passed_after, failed_after = run_compliance(family, bundle, updated_triggers, updated_reqs)
    assert passed_after, f"Expected post-fix pass, still failing: {failed_after}"
    assert updated_triggers == TRIGGERS_T5, "Triggers should not have changed"

    updated_spec = next(
        r["document_spec"] for r in updated_reqs if r["requirement_id"] == "req_federal_tax_return"
    )
    assert updated_spec["document_type"] == "federal_tax_return_2024.pdf", (
        f"Expected 2024 doc type, got: {updated_spec['document_type']!r}"
    )
...
if __name__ == "__main__":
    tests = [
        ...
        ("Test 4 — run_from_saved_feedback bridge (app→agent integration)",    test_run_from_saved_feedback),
        ("Test 5 — wrong tax year in req (2023→2024, family_1/bundle_1)",      test_wrong_tax_year_fix),
    ]
